...
# subprocess.run('python ' + side_detection_path+ 'detect.py --shape-predictor ' + side_detection_path + 'left_custom_weights.dat --image '+side_detection_path+side_picture+'.jpg', shell= True)
...
# subprocess.run('python ' + front_detection_path+'/detect_face_features.py --shape-predictor '+ front_detection_path + '/shape_predictor_68_face_landmarks.dat --image ' + front_detection_path +'images/'+front_picture+'.jpg', shell= True)
...
subprocess.run('cd C:\Program Files\Blender Foundation\Blender 2.82 && blender --python '+cur_path+'/main.py -o ' + cur_path + '/output -f 2', shell=True)